...
export const getProductID = createAsyncThunk(
  ...
  async (id, { rejectWithValue }) => {
    try {
      const response = getProductsByID();
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(uploadAddProduct.rejected, (state, action) => {
        ...
        state.message = action.payload; // Error message from the server
      })
      .addCase(uploadAddProduct.fulfilled, (state, action) => {
        state.productLoading = false;
        state.productSuccess = true;
        state.product.push(action.payload); // Assuming action.payload is the added product
