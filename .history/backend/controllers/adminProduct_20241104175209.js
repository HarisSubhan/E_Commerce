...
const adminAddProduct = AsyncHandler(async (req, res) => {
  ...
  const {
    ...
    title,
    pra,
    category, // Assuming category is the ObjectId from the frontend
    detail,
    description,
    date,
    price,
  ...
  if (
    ...
    res.status(400);
    throw new Error("Enter all relevant fields");
  }

  try {
    // Check if the category exists
    const existingCategory = await AddCategory.findById(category);
    if (!existingCategory) {
